...
export default function AgeCalculator() {
    ...
    const calculateAge = (e: React.FormEvent) => {
        ...
        if (years < 13) {
            lifeStage = 'Childhood';
            description = 'A time of learning, growth, and discovery. Your foundation is being built.';
        } else if (years < 20) {
            lifeStage = 'Adolescence';
            description = 'A transformative period of self-discovery, identity formation, and emotional development.';
        } else if (years < 30) {
            lifeStage = 'Young Adulthood';
            description = 'A time of exploration, career building, and establishing independence.';
        } else if (years < 40) {
            lifeStage = 'Adulthood';
            description = 'A period of consolidation, family building, and professional growth.';
        } else if (years < 50) {
            lifeStage = 'Middle Adulthood';
            description = 'A time of reflection, mentorship, and deeper life purpose.';
        } else if (years < 60) {
            lifeStage = 'Late Adulthood';
            description = 'A period of wisdom, contribution, and spiritual deepening.';
        } else if (years < 70) {
            lifeStage = 'Senior Years';
            description = 'A time of legacy, wisdom sharing, and enjoying life\'s fruits.';
        } else {
            lifeStage = 'Golden Years';
            description = 'A celebration of a life well-lived, with wisdom and peace.';
        }
        ...
        const astrologicalInsight = astrologicalInsights[years % astrologicalInsights.length];
        ...
        setResult({
            ...
            totalHours,
            totalMinutes,
            nextBirthday: nextBirthday.toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }),
                ...
                <Reveal delay={0.3}>
                    <div className="max-w-3xl mx-auto">
                        <p className="text-base text-ink-600 leading-relaxed">
                            Have you ever paused for a moment and thought exactly how many days I have lived? 
                            Or maybe you're trying to figure out the exact age gap between you and your partner? 
                            That's where an <span className="font-semibold text-primary-600">age calculator</span> 
                            comes in. It's not just about numbers, it's about knowing you better.
                        </p>
                    </div>
                </Reveal>
            </div>
                    ...
                    <Reveal>
                        <div className="bg-ink-50 rounded-2xl p-6 md:p-8 border border-ink-100">
                            <div className="flex items-center gap-3 mb-5">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-ink-900">Calculate your Age here</h2>
                                    <p className="text-xs text-ink-500">Enter your birth details to calculate your exact age</p>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-ink-700 mb-2">
                                        Current Age or Age as of <span className="text-danger-500">*</span>
                                    </label>
                                    <div className="grid grid-cols-3 gap-3">
                                        <div>
                                            <select
                            ...
                            {result && (
                                ...
                                >
                                    ...
                                    <div className="bg-gradient-to-br from-primary-600 to-accent-500 rounded-xl p-6 text-center text-white">
                                        <div className="text-sm font-semibold opacity-90">Your Exact Age</div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        <div className="bg-white rounded-xl p-3 text-center border border-ink-100">
                                            ...
                                            <div className="text-lg font-bold text-ink-900">{result.totalDays.toLocaleString()}</div>
                                        </div>
                                        <div className="bg-white rounded-xl p-3 text-center border border-ink-100">
                                            <div className="text-xs text-ink-500">Total Weeks</div>
                                            <div className="text-lg font-bold text-ink-900">{result.totalWeeks.toLocaleString()}</div>
                                        </div>
                                        <div className="bg-white rounded-xl p-3 text-center border border-ink-100">
                                            <div className="text-xs text-ink-500">Total Months</div>
                                            <div className="text-lg font-bold text-ink-900">{result.totalMonths.toLocaleString()}</div>
                                        </div>
                                        <div className="bg-white rounded-xl p-3 text-center border border-ink-100">
                                            <div className="text-xs text-ink-500">Total Hours</div>
                                            <div className="text-lg font-bold text-ink-900">{result.totalHours.toLocaleString()}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-ink-50 rounded-xl p-4 border border-ink-100">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <div className="text-xs text-ink-500">Next Birthday</div>
                                                <div className="text-sm font-semibold text-ink-900">{result.nextBirthday}</div>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-xs text-ink-500">Days Until</div>
                                                <div className="text-sm font-bold text-primary-600">{result.daysUntilNextBirthday} days</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-gradient-to-br from-accent-50 to-primary-50 rounded-xl p-4 border border-accent-200">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="text-sm font-bold text-ink-900">Life Stage: {result.lifeStage}</span>
                                        </div>
                                    ...
                                    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-4 border border-purple-200">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="text-sm font-bold text-ink-900">Astrological Insight</span>
                                        </div>
                                        <p className="text-sm text-ink-600">{result.astrologicalInsight}</p>
                                    </div>
                    ...
                    <Reveal delay={0.1}>
                        <div className="bg-white rounded-2xl p-8 border border-ink-100 w-full">
                            <p className="text-ink-700 leading-relaxed">
                                A date of birth calculator is like your personal timeline tracker; it instantly tells 
                                you exactly how long you've been alive by calculating the time difference between your 
                                birth date and today's date. Think of it as your digital shortcut to knowing your real 
                                age, down to the last day.
                            </p>
                            <div className="mt-4 p-4 bg-accent-50 rounded-xl border border-accent-200">
                                <p className="text-sm text-ink-700">
                                    <span className="font-semibold">ðŸ’¡ This tool</span> isn't just doing basic math. It 
                                    smartly factors in leap years, different month lengths, and even presents your age 
                                    in various formats, be it days, weeks, months, or years.
                                </p>
                            </div>
                            <div className="mt-4 p-4 bg-primary-50 rounded-xl border border-primary-200">
                                <p className="text-sm text-ink-700">
                                    <span className="font-semibold">âœ¨ Designed for India,</span> taking into account 
                                    cultural nuances like Indian Standard Time and traditional muhurats.
                                </p>
                            </div>
                        </div>
                    </Reveal>
                ...
                <div className="mt-8 w-full">
                    <Reveal delay={0.1}>
                        <div className="bg-ink-50 rounded-2xl p-8 border border-ink-100 w-full">
                            <p className="text-ink-700 leading-relaxed">
                                When you input your date of birth day, month, and year, the calculator does a lot 
                                behind the scenes. It takes into account leap years (yes, those extra days every four 
                                years), months with different lengths, and the exact current date.
                            </p>
                            <div className="mt-4 grid md:grid-cols-2 gap-4">
                                <div className="bg-white rounded-xl p-4 border border-ink-100">
                                    <h4 className="text-sm font-bold text-ink-900 mb-2">The Basic Formula</h4>
                                    <p className="text-sm text-ink-600">
                                        Current Year - Birth Year gives you a rough age. But the calculator checks 
                                        whether your birthday has passed this year. If not, it subtracts a year and 
                                        then adds the exact months and days left until your next birthday.
                                    </p>
                                </div>
                                <div className="bg-white rounded-xl p-4 border border-ink-100">
                                    <h4 className="text-sm font-bold text-ink-900 mb-2">Advanced Features</h4>
                        ...
                        <div className="bg-white rounded-2xl p-8 border border-ink-100 w-full">
                            <div className="grid md:grid-cols-3 gap-4">
                                <div className="bg-ink-50 rounded-xl p-4">
                                    ...
                                    <p className="text-sm text-ink-600">
                                        Reflects the broader life phases you've experienced. In Vedic astrology, 
                                        these phases align with planetary periods (dashas) that influence your 
                                        personal growth.
                                    </p>
                                </div>
                                <div className="bg-ink-50 rounded-xl p-4">
                                    <h4 className="text-sm font-bold text-ink-900 mb-2">Months & Days</h4>
                                    <p className="text-sm text-ink-600">
                                        Add even more depth to your journey. Realizing you've lived exactly 10,000 
                                        days can inspire meaningful celebrations and reflection.
                                    </p>
                                </div>
                                <div className="bg-ink-50 rounded-xl p-4">
                                    <h4 className="text-sm font-bold text-ink-900 mb-2">Practical Use</h4>
                                    <p className="text-sm text-ink-600">
                                        Useful for LIC maturity, PF withdrawal, visa applications, school admissions, 
                                        and legal forms where precise age is required.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div className="container-8xl">
                ...
                <div className="mt-8 w-full">
                    <Reveal delay={0.1}>
                        <div className="bg-ink-50 rounded-2xl p-8 border border-ink-100 w-full">
                            <p className="text-ink-700 leading-relaxed">
                                Most date of birth calculators in India automatically default to Indian Standard Time (IST). 
                                But when you're dealing with international paperwork or services abroad, it's smart to 
                                switch the time zone accordingly.
                            </p>
                            <div className="mt-4 grid md:grid-cols-2 gap-4">
                                <div className="bg-white rounded-xl p-4 border border-ink-100">
                                    <h4 className="text-sm font-bold text-ink-900 mb-2">Astrological Importance</h4>
                                    <p className="text-sm text-ink-600">
                                        Your entire birth chartâ€”your lagna (ascendant), rashi (moon sign), and 
                                        predictions can shift with just an hour's difference. That's why astrologers 
                                        always ask for your exact time and place of birth.
                                    </p>
                                </div>
                                <div className="bg-white rounded-xl p-4 border border-ink-100">
                                    <h4 className="text-sm font-bold text-ink-900 mb-2">Daylight Saving</h4>
                                    <p className="text-sm text-ink-600">
                                        While India doesn't follow daylight saving, countries like the U.S. and UK 
                                        adjust their clocks twice a year. Advanced tools handle these shifts automatically.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="mt-8 grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                    <Reveal delay={0.1}>
                        <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:shadow-lg transition-all">
                            ...
                            <h3 className="text-lg font-bold text-ink-900 mb-2">Subtract Method</h3>
                            <p className="text-sm text-ink-500">
                                Subtract your birth year from the current year. If your birthday hasn't passed 
                                this year, subtract one year.
                            </p>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.2}>
                        <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:shadow-lg transition-all">
                            ...
                            <h3 className="text-lg font-bold text-ink-900 mb-2">Memory Method</h3>
                            <p className="text-sm text-ink-500">
                                Think back to a big moment in your life. Use it as a mental bookmark to quickly 
                                figure out your current age by counting forward from that year.
                            </p>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.3}>
                        <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:shadow-lg transition-all">
                            ...
                            <h3 className="text-lg font-bold text-ink-900 mb-2">Decade Method</h3>
                            <p className="text-sm text-ink-500">
                                Count the full decades you've lived and add the remaining years for a quick estimate.
                            </p>
                        </div>
                    </Reveal>
                </div>
                ...
                <div className="mt-8 grid md:grid-cols-2 gap-6 max-w-6xl mx-auto">
                    <Reveal delay={0.1}>
                        <div className="bg-ink-50 rounded-2xl p-6 border border-ink-100 hover:shadow-lg transition-all">
                            <h3 className="text-xl font-bold text-ink-900 mb-3">Western System</h3>
                            <p className="text-sm text-ink-600 mb-3">
                                You count from the day you were born, and each birthday adds a year. This is the 
                                global standard most birth date calculator tools follow.
                            </p>
                            <div className="bg-white rounded-xl p-3 border border-ink-100">
                                <p className="text-xs text-ink-500">Used worldwide for official documents</p>
                            </div>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.2}>
                        <div className="bg-ink-50 rounded-2xl p-6 border border-ink-100 hover:shadow-lg transition-all">
                            <h3 className="text-xl font-bold text-ink-900 mb-3">Indian (Vedic) System</h3>
                            <p className="text-sm text-ink-600 mb-3">
                                Takes into account lunar cycles, planetary positions, and nakshatras. Astrologers 
                                use multiple systems for incredible spiritual and cosmic precision.
                            </p>
                            <div className="bg-white rounded-xl p-3 border border-ink-100">
                                <p className="text-xs text-ink-500">Used for astrological and spiritual purposes</p>
                            </div>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.3}>
                        <div className="bg-ink-50 rounded-2xl p-6 border border-ink-100 hover:shadow-lg transition-all">
                            <h3 className="text-xl font-bold text-ink-900 mb-3">Chinese System</h3>
                            <p className="text-sm text-ink-600 mb-3">
                                You're considered one year old the moment you're born, and everyone adds a year 
                                on the Chinese New Year, not on their birthday.
                            </p>
                            <div className="bg-white rounded-xl p-3 border border-ink-100">
                                <p className="text-xs text-ink-500">Chinese age might be 1-2 years ahead of Western age</p>
                            </div>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.4}>
                        <div className="bg-ink-50 rounded-2xl p-6 border border-ink-100 hover:shadow-lg transition-all">
                            <h3 className="text-xl font-bold text-ink-900 mb-3">Vikram Samvat Calendar</h3>
                            <p className="text-sm text-ink-600 mb-3">
                                Still followed in many parts of India, dates back to 57 BCE. According to this 
                                calendar, we're in a completely different year than the Gregorian calendar.
                            </p>
                            <div className="bg-white rounded-xl p-3 border border-ink-100">
                                <p className="text-xs text-ink-500">Used for religious and cultural ceremonies</p>
                            </div>
                        </div>
                    </Reveal>
                </div>
                ...
                <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
                    <Reveal delay={0.1}>
                        <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:border-accent-200 hover:shadow-lg transition-all text-center">
                            ...
                            <h3 className="text-sm font-bold text-ink-900">Career Planning</h3>
                            <p className="text-xs text-ink-500">Understand career cycles and optimal timing for job changes</p>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.2}>
                        <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:border-accent-200 hover:shadow-lg transition-all text-center">
                            ...
                            <h3 className="text-sm font-bold text-ink-900">Relationship Compatibility</h3>
                            <p className="text-xs text-ink-500">Precise age calculations reveal compatibility patterns</p>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.3}>
                        <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:border-accent-200 hover:shadow-lg transition-all text-center">
                            ...
                            <h3 className="text-sm font-bold text-ink-900">Family Planning</h3>
                            <p className="text-xs text-ink-500">Crucial information for medical considerations and ceremonies</p>
                        </div>
                    </Reveal>
                    ...
                    <Reveal delay={0.4}>
                        <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:border-accent-200 hover:shadow-lg transition-all text-center">
                            ...
                            <h3 className="text-sm font-bold text-ink-900">Milestone Planning</h3>
                            <p className="text-xs text-ink-500">Plan celebrations and align rituals with cosmic timing</p>
                        </div>
                    </Reveal>
                </div>
            </div>
            ...
            <div className="relative container-8xl text-center">
                ...
                <Reveal delay={0.1}>
                    <h2 className="mt-4 text-3xl font-bold text-white sm:text-4xl lg:text-5xl">
                        Ready to discover your <span className="gradient-text-light">exact age?</span>
                    </h2>
                </Reveal>

                <Reveal delay={0.2}>
                    <p className="mt-4 text-lg text-ink-400 max-w-2xl mx-auto">
                        Calculate your exact age now and unlock insights into your life's journey and cosmic alignment.
                    </p>
                </Reveal>
