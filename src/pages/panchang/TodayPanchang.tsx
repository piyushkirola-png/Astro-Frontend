...
const getTodayPanchang = (city: string) => {
    ...
    return {
        ...
        tarabalam: ['Krittika', 'Mrigashirsha', 'Punarvasu', 'Ashlesha', 'Uttara Phalguni', 'Chitra', 'Vishakha', 'Jyeshtha', 'Uttara Ashadha', 'Dhanishta', 'Purva Bhadrapada'],
        chandrabalam: ['Krittika', 'Mrigashirsha', 'Punarvasu', 'Ashlesha', 'Uttara Phalguni', 'Chitra', 'Vishakha', 'Jyeshtha', 'Uttara Ashadha', 'Dhanishta', 'Purva Bhadrapada'],
        planets: [
            { name: 'Ascendant', rashi: 'Leo', longitude: '21Â°4â€²55â€³', nakshatra: 'Purva Phalguni', pada: '3' },
            { name: 'SUN', rashi: 'Leo', longitude: '22Â°8â€²13â€³', nakshatra: 'Purva Phalguni', pada: '3' },
            { name: 'MOON', rashi: 'Cancer', longitude: '24Â°34â€²35â€³', nakshatra: 'Ashlesha', pada: '3' },
            { name: 'MERCURY', rashi: 'Virgo', longitude: '2Â°56â€²6â€³', nakshatra: 'Uttara Phalguni', pada: '2' },
            { name: 'VENUS', rashi: 'Libra', longitude: '4Â°53â€²19â€³', nakshatra: 'Chitra', pada: '4' },
            { name: 'MARS', rashi: 'Gemini', longitude: '24Â°12â€²7â€³', nakshatra: 'Punarvasu', pada: '2' },
            { name: 'JUPITER', rashi: 'Cancer', longitude: '21Â°7â€²44â€³', nakshatra: 'Ashlesha', pada: '2' },
            { name: 'SATURN', rashi: 'Pisces', longitude: '18Â°57â€²33â€³', nakshatra: 'Revati', pada: '1' },
            { name: 'RAHU', rashi: 'Aquarius', longitude: '4Â°38â€²48â€³', nakshatra: 'Dhanishta', pada: '4' },
            { name: 'KETU', rashi: 'Leo', longitude: '4Â°38â€²48â€³', nakshatra: 'Magha', pada: '2' }
        ]
    };
};
...
export default function TodayPanchang() {
    ...
    const [panchangData, setPanchangData] = useState(getTodayPanchang('New Delhi, Delhi, India'));
                ...
                <Reveal delay={0.3}>
                    <div className="max-w-3xl mx-auto">
                        <p className="text-base text-ink-600 leading-relaxed">
                            Panchang daily tracks the cosmic energy of each day to determine key events such as 
                            marriage, new business venture, naming ceremony, travel, and fasting. 
                            <span className="font-semibold text-primary-600"> Panchang</span> is derived from the 
                            Sanskrit words panch, meaning five, and ang, meaning limbs.
                        </p>
                    </div>
                </Reveal>
            </div>
                        ...
                        <div className="bg-gradient-to-br from-ink-50 to-primary-50 rounded-2xl p-6 md:p-8 border border-ink-100">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                                ...
                                <div className="bg-white rounded-xl p-3 text-center border border-ink-100">
                                    ...
                                    <div className="text-sm font-bold text-ink-900">{panchangData.shakaSamvat}</div>
                                </div>
                                <div className="bg-white rounded-xl p-3 text-center border border-ink-100">
                                    <div className="text-xs text-ink-500">Vikram Samvat</div>
                                    <div className="text-sm font-bold text-ink-900">{panchangData.vikramSamvat}</div>
                                </div>
                            </div>
                                ...
                                <div className="grid sm:grid-cols-2 gap-2">
                                    {panchangData.inauspiciousTimings.map((timing: any, i: number) => (
                                        <div key={i} className="bg-red-50 rounded-lg p-3 border border-red-200">
                                            <div className="text-xs font-semibold text-red-700">{timing.name}</div>
                                            <div className="text-xs text-red-600">From {timing.from} To {timing.to}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-4 mb-6">
                                <div className="bg-green-50 rounded-xl p-4 border border-green-200">
                                    <h4 className="text-sm font-bold text-green-700 mb-2">Tarabalam</h4>
                                    <div className="flex flex-wrap gap-1">
                                        {panchangData.tarabalam.map((nakshatra: string, i: number) => (
                                            <span key={i} className="text-xs bg-white text-green-700 px-2 py-0.5 rounded-full border border-green-200">
                                                {nakshatra}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
                                    <h4 className="text-sm font-bold text-blue-700 mb-2">Chandrabalam</h4>
                                    <div className="flex flex-wrap gap-1">
                                        {panchangData.chandrabalam.map((nakshatra: string, i: number) => (
                            ...
                            {/* Planetary Positions */}
                            <div>
                                <h3 className="text-lg font-bold text-ink-900 mb-3">Planetary Positions</h3>
                                    ...
                                    <table className="w-full bg-white rounded-xl border border-ink-100">
                                        <thead>
                                            <tr className="bg-gradient-to-r from-primary-600 to-accent-500">
                                                <th className="text-left py-2 px-3 text-white font-semibold text-xs">Planets</th>
                                                <th className="text-left py-2 px-3 text-white font-semibold text-xs">Rashi</th>
                                                <th className="text-left py-2 px-3 text-white font-semibold text-xs">Longitude</th>
                                                <th className="text-left py-2 px-3 text-white font-semibold text-xs">Nakshatra</th>
                                                <th className="text-left py-2 px-3 text-white font-semibold text-xs">Pada</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {panchangData.planets.map((planet: any, i: number) => (
                                                <tr key={planet.name} className={i % 2 === 0 ? 'bg-ink-50' : 'bg-white'}>
                                                    <td className="py-2 px-3 text-xs font-semibold text-ink-900">{planet.name}</td>
                                                    <td className="py-2 px-3 text-xs text-ink-600">{planet.rashi}</td>
                                                    <td className="py-2 px-3 text-xs text-ink-600">{planet.longitude}</td>
                                                    <td className="py-2 px-3 text-xs text-ink-600">{planet.nakshatra}</td>
                                                    <td className="py-2 px-3 text-xs text-ink-600">{planet.pada}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
    ...
    const importanceItems = [
        {
            icon: Heart,
            title: 'Marriage',
            description: 'An auspicious day is always selected according to Vedic astrology. It considers Tithi, Yoga, Vaar, and Nakshatra to determine the best day and time for marriage.',
            color: 'from-pink-500 to-rose-500'
        },
        {
            icon: Briefcase,
            title: 'Business',
            description: 'Inaugurating an office, starting a new venture, or signing off on important papers at the right muharat brings success and cuts off obstacles.',
            color: 'from-blue-500 to-cyan-500'
        },
        {
            icon: Baby,
            title: 'Naming Ceremony',
            description: 'Determines a child\'s Nakshatra and zodiac, which describes a child\'s personality, strengths, and the life path they will follow.',
            color: 'from-green-500 to-emerald-500'
        },
        {
            icon: Compass,
            title: 'Travel',
            description: 'Lets you know the right time to leave home for utmost safety and security. Determines the Nakshatra and the best days for a hassle-free journey.',
            color: 'from-purple-500 to-indigo-500'
        },
        {
            icon: Moon,
            title: 'Fasting',
            description: 'Observing fast in Indian culture also depends upon the Panchang. It determines the correct Tithi that enables a native to derive the full benefits of fasting.',
                    ...
                    <div className="mt-8 max-w-6xl mx-auto">
                        <div className="bg-gradient-to-br from-primary-50 to-accent-50 rounded-2xl p-6 border border-accent-200">
                            <p className="text-sm text-ink-700 text-center">
                                <span className="font-semibold">ðŸ’¡ Panchang</span> is derived from the Sanskrit words 
                                <span className="font-semibold"> panch</span> (five) and 
                                <span className="font-semibold"> ang</span> (limbs). The Panchang daily considers five 
                                elements: Tithi, Vaar, Nakshatra, Yoga, and Karana. It determines the energy and 
                                auspiciousness of every day.
                            </p>
                        </div>
                    </div>
                </Reveal>
                    ...
                    {elements.map((element, i) => (
                        <Reveal key={element.name} delay={i * 0.1}>
                            <div className="bg-white rounded-2xl p-6 border border-ink-100 hover:border-accent-200 hover:shadow-lg transition-all text-center">
                                ...
                                <h3 className="text-lg font-bold text-ink-900 mb-1">{element.name}</h3>
            ...
            <div className="relative container-8xl text-center">
                ...
                <Reveal delay={0.2}>
                    <p className="mt-4 text-lg text-ink-400 max-w-2xl mx-auto">
                        Plan your day with cosmic wisdom. Check auspicious timings, planetary positions, and more.
                    </p>
                </Reveal>
